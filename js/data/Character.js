...
const nodeId in this.skillTree.nodes) { this.skills[nodeId] = { level: 0, unlocked: false }; } const rootId = this.skillTree.root; if (rootId && this.skills[rootId]) { this.skills[rootId].level = 1; this.skills[rootId].unlocked = true; } } }
    ...
    getEquipmentStatBonus(stat) { let bonus = 0; for (const slot in this.equipment) { const item = this.equipment[slot]; if (item && item.stats && item.stats[stat]) { bonus += item.stats[stat]; } } retu
rn bonus; }
    getSkillBonus(stat) { let bonus = 0; for (const skillId in this.skills) { const skill = this.skills[skillId]; if (skill.unlocked && skill.level > 0) { const node = this.skillTree.getNode(skillId); if (node && node.getEffect() && node.getEffect()[stat]) { bonus += node.getEffect()[stat] * skill.level; } } } return bonus; }
...
ll) return false; if (!this.abilities.includes(spellId)) return false; const manaCost = spell.baseManaCost * GameConfig.spells.manaCostMultiplier; if (this.stats.mana < manaCost) return false; const now = Date.now(); if (this.spellCooldowns[spellId] && now < this.spellCooldowns[spellId]) return false; this.stats.mana -= manaCost; const cooldown = spell.baseCooldown * GameConfig.spells.cooldownReduction; this.spellCooldowns[spellId] = now + cooldown; this.isCasting = true; this.currentSpell = spell; switch (spell.type) { case 'damage': this.castDamageSpell(spell, targets); break; case 'heal': this.castHealSpell(spell, targets); break; case 'aoe': this.castAoESpell(spell, targets); break; case 'buff': this.castBuffSpell(spell, targets); break; case 'debuff': this.castDebuffSpell(spell, targets); break; } setTimeout(() => { this.isCasting = false; this.currentSpell = null; }, 500); return true; }
    ...
    castBuffSpell(spell, targets) { if (Array.isArray(targets)) { targets.forEach(target => { if (target && target.isAlive) this.applyBuff(target, spell); }); } else if (targets) { this.applyBuff(targets
, spell); } else { this.applyBuff(this, spell); } }
...
illId]; if (!node) return false; if (this.skills[skillId] && this.skills[skillId].unlocked) return false; if (this.skillPoints < 1) return false; for (const req of node.requirements || []) { const requiredSkill = this.skills[req.skill]; if (!requiredSkill || !requiredSkill.unlocked || requiredSkill.level < req.level) return false; } const depth = this.calculateSkillDepth(skillId); if (depth > GameConfig.skills.maxSkillDepth) return false; return true; }
    calculateSkillDepth(skillId) { const node = this.skillTree.getNode(skillId); if (!node || !node.definition.requirements || node.definition.requirements.length === 0) return 0; let maxDepth = 0; for (const req of node.definition.requirements) { const depth = this.calculateSkillDepth(req.skill); maxDepth = Math.max(maxDepth, depth + 1); } return maxDepth; }
    unlockSkill(skillId) { if (!this.canUnlockSkill(skillId)) return false; const node = this.skillTree.getNode(skillId); if (!node) return false; this.skills[skillId] = { level: 1, unlocked: true }; this.skillPoints--; this.updateStats(); node.unlock(); return true; }
    upgradeSkill(skillId) { const skill = this.skills[skillId]; if (!skill || !skill.unlocked) return false; const node = this.skillTree.getNode(skillId); if (!node) return false; if (skill.level >= node.maxLevel) return false; if (this.skillPoints < 1) return false; skill.level++; this.skillPoints--; this.updateStats(); node.levelUp(); return true; }
